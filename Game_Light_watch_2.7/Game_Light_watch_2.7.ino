...
void setup() {
  ...
  RTC.CTRLA = RTC_PRESCALER_DIV64_gc | RTC_RTCEN_bm | RTC_RUNSTDBY_bm; //PRESCALER = (32768/PERIOD)-1
...
void loop() {
  ...
  if ((interruptTimer - timer) > sleepDelayTimer && totalSeconds == 0 && !resumeOperation) { // prepare to sleep
...
void displayTime(void) { // draws time in display
  drawTime(64, 1, hours);
