...
void loop() {
  ...
  if (BUTTONLOW && (interruptTimer - timer) > 800)  options();
  ...
  if ((interruptTimer - timer) > 4000 && totalSeconds == 0 && !resumeOperation) {
    timer = interruptTimer;

    if (!BATTCHR) {//if batt is charging do not enter sleep
      goToSleep();
    } else {
      displayTimeBatt();
    }
...
void displayTimeBatt (void) {
  ...
  if (!BATTCHR) {// draws battery or charging icon if connected to usb.
    drawSprite(0, 3, battSprite, 8, 1, SPRITENORMAL);
  } else {
    drawSprite(0, 3, chargingSprite, 8, 1, SPRITENORMAL);
  }
}
...
ISR(RTC_PIT_vect) {
  ...
  if (seconds++, seconds > 59) {// acutal time keeping
    ...
    if (minutes++, minutes > 59) {
      minutes = 0;
      //seconds = 12;  // internal crystal is slow to keep time accurate, compensate that adding some seconds every hour
