...
void loop() {
  ...
  if ((interruptTimer - timer) > 4000 && minutesCounter == 0) {
    clearScreen();
...
ISR(PORTA_PORT_vect) {
  ...
  if (PORTA.INTFLAGS & PIN6_bm) {
    ...
    // PORTA.OUT &= ~ PIN7_bm; //mosfet low, ON
    shouldWakeUp = true;
    resumeOperation = true;
    timer = interruptTimer;
  }
...
ISR(RTC_PIT_vect) {
  ...
  if (seconds++, seconds > 59) {// acutal time keeping
    ...
    if (minutes++, minutes > 59) {
      ...
      if (hours++, hours > 23) {
        hours = 0;
        seconds += 45; // internal crystal is slow to keep time accurate, compensate that adding some seconds every hour
      }
    }
  }
