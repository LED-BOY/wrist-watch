...
void setup() {
  ...
  while ((interruptTimer < 1000) && BUTTONLOW) ledPWM = true; // when the device is reseted turn on led
  ledPWM = false;
}
...
void loop() {
  ...
  if ((interruptTimer - timer) > sleepDelayTimer) { // prepare to sleep
    timer = interruptTimer;
    oled.dimScreen();
    screenDimmed = true;

    if (totalSeconds == 0 && !resumeOperation) {
      oled.clearScreen();
...
void displayTime(uint8_t hourPos, uint8_t minutesPos) { // draws time in display
  drawTime(hourPos, 1, hours);
  drawTime(minutesPos, 1, minutes);
}
...
ISR(PORTA_PORT_vect) {
  ...
  timer = 0;
  sleep_disable();
  _PROTECTED_WRITE(WDT.CTRLA, WDT_PERIOD_8KCLK_gc);// enable watchdog 8 sec. aprox.
}
